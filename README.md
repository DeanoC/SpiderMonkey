...
Spider Monkey is the standalone Spiderweb worker process. In this first pass it does not talk to Spiderweb directly. It is given a mounted Spiderweb workspace folder and uses the filesystem inside that mount as its contract.
...
- Prints a startup summary with the paths it discovered.
...
- By default, processes queued jobs by writing `running`/`done`, filling `result.txt` and `log.txt`, and mirroring the latest reply into `chat/control/reply`.
...
This is still a scaffold for the external worker split. The current worker logic is deliberately simple and deterministic so the external-process filesystem loop can be exercised end-to-end before provider integration is added.
